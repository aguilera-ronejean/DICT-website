...
if (isset($_GET['edit'])) {     // puts selected row into textbox for it to be editted/deleted
    ...
    $sql = "SELECT * FROM accomplishment_general WHERE id = '$id'";
    ...
    $name_Wifi_Site = $row['name_Wifi_Site'];
    $site_Type = $row['site_Type'];
    $contract_Type = $row['contract_Type'];
    $cir_Mbps = $row['cir_Mbps'];
    $access_points = $row['access_points'];
    $date_Tested = $row['date_Tested'];
    $activity_Type = $row['activity_Type'];
    $name_Tower = $row['name_Tower'];
    $pow_tor_Status = $row['pow_tor_Status'];
    $date_NOA = $row['date_NOA'];
    $mode = $row['mode'];
    $renovation_Status = $row['renovation_Status'];
    $contractor = $row['contractor'];
    $estimated_Com_Date = $row['estimated_Com_Date'];
    $completion_Date = $row['completion_Date'];
    $name_Agency = $row['name_Agency'];
    $connectivity_Status = $row['connectivity_Status'];
    $tech_ass_Prov = $row['tech_ass_Prov'];
    $remarks = $row['remarks'];
    $cong_District = $row['cong_District'];
    $GIDA_ELCAC = $row['GIDA_ELCAC'];
    $date_Conducted_gv = $row['date_Conducted_gv'];
    $pnpki_title = $row['pnpki_title'];
    $pnpki_mode = $row['pnpki_mode'];
    $pnpki_date = $row['pnpki_date'];
    $epbls_status = $row['epbls_status'];
    $epbls_OP = $row['epbls_OP'];
    $epbls_Date = $row['epbls_Date'];
    $delivery_Days = $row['delivery_Days'];
    $date_acc_gv = $row['date_acc_gv'];
...
$result_FWTEST = mysqli_query($GLOBALS['conn'], $sql_FWTEST) or die( mysqli_error($conn));;
$row_FWTEST = mysqli_fetch_array($result_FWTEST);
...
$result_FW = mysqli_query($GLOBALS['conn'], $sql_FW) or die( mysqli_error($conn));;
...
$result_GN = mysqli_query($GLOBALS['conn'], $sql_GN) or die( mysqli_error($conn));;
...
$result_eBPLS = mysqli_query($GLOBALS['conn'], $sql_eBPLS) or die( mysqli_error($conn));;
...
$result_PNPKI = mysqli_query($GLOBALS['conn'], $sql_PNPKI) or die( mysqli_error($conn));;
...
<html>
    <head>
    <link rel="stylesheet" href="accomplishmentGeneral1.css" type="text/css">
    <title>General Accomplishments</title>
    </head>
    <body>
        <main>
                                            ...
                                            while($row  = mysqli_fetch_array($result)){ $id = $row['id']; ?>
                                                ...
                                                <tr id="<?php echo $row['id']; ?>"> 
                                                    ...
                                                    <td style="text-align:center">
                                                        <a href="accomplishmentGeneral.php?edit=<?php echo $row["id"]; ?>" class="edit_btn"><i class="fas fa-edit fa-lg"></i>
                                    ...
                                    <center>
                                        <?php
                                            for ($i=1; $i<=$total_pages_FWTEST; $i++) {                                  
                                                echo '<a class="btn btn-warning" href="accomplishmentGeneral.php?page='.$i.'">'.$i.'</a>'; 
                                            }; 
                                        ?>                      
                                    </center> 
                                </table>                
            ...
            <div class="accordion-item">
                ...
                <div class="container">
                    <form method="post" action="accomplishmentGeneralOp.php" enctype="multipart/form-data"> 
